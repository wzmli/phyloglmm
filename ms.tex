...
\begin{equation}
Z^{C}_{i} = (BS \bigotimes I_{q_i \times s})^T Z_{i} ,
\end{equation}

where $BS$ is an $b \times s$ matrix of branch--species relations; $I$ is an identity matrix size $q_i \times s$; $q_i$ is the number of columns contributed by $Z_i$; s is the number of sites; and  $\bigotimes$ is the Kronecker product \ml{cite kronecker}.  
...
We generate test data using a simple glmm framework that varies in complexity in random effect described in Table 1 \ml{macro this}. All simulations and fits were performed with R. 
