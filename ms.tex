...
The most widely used statistical method to model related species data that accounts for phylogenetic structure is the phylogenetic regression (Felsenstein 1985).
...
\ml{where can I add a line about independent contrast? maybe in the discussion?}


Recent years, researchers have begun to adpot these phylogenetic-based methods in various applications. 
Despite the choice of modeling strageties, there are many limitations in the current state of art of statistical phylogenetic analysis (i.e model selection and etc). 
...
In this paper, we propose an alternative, yet straightforward but little-used method of constructing $Z^C$ directly from phylogenetic tree by summing the evolutionary changes that occurred on all of the branches in the phylogeny in its past.
