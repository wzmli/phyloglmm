...
The distribution plot above just shows the amount of change each species evolved compared to the ancestral node.
Thus, the null tree will have the same species level trait distribution, but what we are interested in are the "crosses".
...
pp <- expand.grid(t1 = seq(-10,10,length=21)
  , t2 = seq(-10,10,length=21)    ## non-focal parameter
  , t3 = seq(-3,3,length=9)
)
...
Stats 101, can we see it more clearly if we have more data?
...
pp <- expand.grid(t1 = seq(-10,10,length=9)
  , t2 = seq(-10,10,length=9)    ## non-focal parameter
  , t3 = seq(-10,10,length=9)
)
...
```{r more species, warning=FALSE, echo=FALSE, message=FALSE,cache=TRUE}

ddprop <- data.frame(nspp=3:6, prop=NA)
ddprop$prop[1] <- mean(pp2$LRT)
rep_pp <- pp
for(i in 4:6){
phy <- rtree(n = i)
Vphy <- vcv(phy)

temp_dimension <- expand.grid(rep(1,nrow(rep_pp)),new=seq(-10,10,length=9))
rep_pp <- rbind(rep_pp,rep_pp,rep_pp,rep_pp,rep_pp,rep_pp,rep_pp,rep_pp,rep_pp)
newgrid <- cbind(rep_pp,temp_dimension[,"new"])
colnames(newgrid)[i] <- paste("t",i,sep="_")

newgrid$ll <- sapply(1:nrow(newgrid),function(x){mvtnorm::dmvnorm(newgrid[x,1:i], mean =rep(0,i), sigma = Vphy, log = TRUE)})
c <- qchisq(0.95,df=i-2)

Vnull <- diag(diag(vcv(phy)))

newgrid$null <- sapply(1:nrow(newgrid),function(x){mvtnorm::dmvnorm(newgrid[x,1:i], mean =rep(0,i), sigma = Vnull, log = TRUE)})
...
ddprop$prop[i-2] <- mean(newgrid2$LRT)
print(i)
rep_pp <- newgrid2[,1:i]
}
...
Brute forcing the grid is not a good idea and very inefficient. Can we get away with just sampling the tree?

```{r, sampling, echo=FALSE,cache=TRUE}
ddprop <- data.frame(nspp=3:40, prop=NA)
rep_pp <- pp
for(i in 3:40){
phy <- rtree(n = i)
Vphy <- vcv(phy)

Y_phy <- MASS::mvrnorm(n=10000
  , mu=rep(0,each=i)
  , Sigma=Vphy
)
