...
In such case, researchers often try to find ways to simplify or revert to simpler models (i.e. model reduction or ignore phylogenetic relationships, or both) for practical uses which can lead to both statistical and biological issues \cite{felsenstein1985phylogenies, li2017statistical}. 
\ml{In this paper, we propose an alternative method that can flexibly and computational efficiently model phylogenetic relationships via mixed effect models, highlighting various complexities of phylogenetic relationships through random effects.
...
\ml{move this to the last paragraph in the intro?}

Residual variation (or variation in residuals) is a serious concern for most comparative studies (e.g., \cite{pagel1988recent, kelly2004comparative,ives2007within} and it has different, yet correct interpretations in various phylogenetic comparative literature. 
For example, the standard/classic phylogenetic regression is a correlated-residual model using phylogenetically independent contrasts (PICs), where the residuals evolve as a Brownian motion process \citep{felsenstein1985phylogenies}; in other words, the variation of residuals are phylogenetically correlated.
Many of the more recent approaches -- including phylogenetic generalized linear mixed models (PGLMM) \citep{ives2011generalized}, Pagel's $\lambda$ \citep{pagel1999inferring}, and Blomberg's $K$ \citep{blomberg2003testing} -- are built upon Felsentein's PICs model by making additional sophisticated assumptions to correct for possible bias where separates/partition phylogenetically correlated residual variations into phylogenetically uncorrelated residual variation (observation error or tip variation) and phylogenetic signal (biological/evolutionary process error) \cite{hansen2012interpreting}.
Lastly, the standard statistical residual variation of repeated measures or multiple observations of the same taxon, where phylogenetic signal and tip variation are now both part of the evolutionary process error. 
The last case is straightforward statistically, but rarely seen in the literature and existing platforms are unable to fit multiple observations, or unbalanced design data, or both.
