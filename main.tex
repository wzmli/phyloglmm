...
%\ml{chapter 7 in garamszegi 2014}
...
\ml{inverse VCV for BLUP? an alternative way to find the inverse of the vcv(phy)}
...
\end{equation}
\ml{need a matrix diagram on the linear algebra}
...
Our algorithmic approach is general and could be implemented in a wide range of computational platforms that support independent latent variables. We implemented our approach using the R packages \pkg{lme4} \citep{bates2015fitting} and \pkg{glmmTMB} \ml{citation}.
...
Neither PGLS nor PGLM can multiple observations or predictor level variation \ml{intercept level vs predictor level?}. 
